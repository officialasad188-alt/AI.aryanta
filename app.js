document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- INDEXEDDB HELPER (FIX FOR LOCALSTORAGE QUOTA) ---
    const DB_NAME = 'AryantaDB';
    ...
    const dbHelper = {
        ...
        save: async (dataUrl) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                tx.objectStore(STORE_NAME).put({ id: id, data: dataUrl });
    ...
    document.addEventListener('click', (e) => { if(!e.target.closest('.custom-select-trigger')) closeAllSelects(e); });
    window.addEventListener('scroll', (e) => { if(e.target.classList && e.target.classList.contains('custom-select-options')) return; closeAllSelects(null); }, true);
    ...
    window.copyTxt = (txt) => { navigator.clipboard.writeText(txt).then(() => { showToast("Copied to Clipboard"); }); };
    ...
    // --- NAVIGATION & TOOLS REDIRECT ---
    const nav = (target) => {
        Object.values(screens).forEach(s => s.style.display = 'none');
        if(screens[target]) { screens[target].style.display = 'block'; window.scrollTo(0,0); }
    };
    ...
    // UPDATED TOOLS REDIRECTS
    document.getElementById('btn-chat-box').onclick = () => { window.location.href = "chat.html"; };
    document.getElementById('btn-tools-section').onclick = () => { window.location.href = "https://example.com"; };
    document.querySelectorAll('.back-btn').forEach(btn => { btn.onclick = () => nav('main'); });

    // --- NOTIFICATIONS ---
    const checkNotifications = () => {
        const today = new Date();
        const d = today.getDate(); const m = today.getMonth();
        const name = localStorage.getItem('user-name') || 'Brother';
        if (d === 18 && m === 7) { alert(`🎂 Happy Birthday ${name}! 🎉\nWishing you a fantastic year!`); return; }
        const holidays = { "25-11": "Merry Christmas", "1-0": "Happy New Year", "26-0": "Happy Republic Day" };
        const key = `${d}-${m}`; if(holidays[key]) { alert(`🎉 ${holidays[key]}!`); }
    };
    ...
    [1,2,3,4,5,6,7,8,9,'C',0,'⌫'].forEach(k => {
        const btn = document.createElement('div'); btn.className = 'pin-key'; btn.innerText = k;
    ...
    const openPin = (mode, callback, desc="Enter PIN") => {
        ...
        pinMode = mode; pinCallback = callback; currentInput = ""; failedAttempts = 0;
        pinTitle.innerText = mode === 'set' ? "Set New PIN" : "Security Check";
        pinDesc.innerText = desc; pinDesc.style.color = "var(--text-muted)";
        pinPad.style.display = 'grid'; pinDots.style.display = 'flex'; pinTimer.style.display = 'none';
        forgotBtn.style.display = (mode === 'login') ? 'block' : 'none';
        document.getElementById('cancel-pin').style.display = (mode === 'login') ? 'none' : 'block';
        renderPinDots(); pinScreen.style.display = 'flex';
    };

    forgotBtn.onclick = () => {
        const code = prompt("Enter Security System Code:");
        if(code === "992345") {
            localStorage.removeItem('user-pin'); localStorage.removeItem('pin-lock-until'); localStorage.removeItem('face-enabled');
            showToast("System Verified: Unlocked"); alert("System Lock: Access Granted.\nPIN & Face ID have been reset.");
            pinScreen.style.display = 'none'; document.getElementById('main-app').style.display = 'block';
        } else { alert("Security Code Invalid.\nSystem Lock cannot open."); }
    };

    document.getElementById('cancel-pin').onclick = () => { pinScreen.style.display = 'none'; currentInput = ""; };

    const renderPinDots = () => {
        pinDots.innerHTML = '';
        for(let i=0; i<4; i++) {
            const d = document.createElement('div'); d.className = 'pin-dot' + (i < currentInput.length ? ' filled' : ''); pinDots.appendChild(d);
        }
    };

    const updatePinInput = (val) => {
        if(val === 'clear') currentInput = ""; else if(val === 'back') currentInput = currentInput.slice(0, -1); else if(currentInput.length < 4) currentInput += val;
        renderPinDots(); if(currentInput.length === 4) setTimeout(() => handlePinSubmit(), 200);
    };

    const isLocked = () => {
        const lockUntil = localStorage.getItem('pin-lock-until'); if(!lockUntil) return false;
    ...
    const startLockdownTimer = () => {
        ...
        pinScreen.style.display = 'flex'; pinPad.style.display = 'none'; pinDots.style.display = 'none';
        forgotBtn.style.display = 'none'; document.getElementById('cancel-pin').style.display = 'none';
        pinTimer.style.display = 'block'; pinTitle.innerText = "System Locked"; pinDesc.innerText = "Too many incorrect attempts.";
        pinDesc.style.color = "var(--danger)";
        clearInterval(lockInterval);
        lockInterval = setInterval(() => {
            const now = new Date().getTime(); const left = Math.ceil((lockUntil - now) / 1000);
            if(left <= 0) {
                clearInterval(lockInterval); localStorage.removeItem('pin-lock-until');
                pinPad.style.display = 'grid'; pinDots.style.display = 'flex'; pinTimer.style.display = 'none';
                forgotBtn.style.display = 'block'; pinTitle.innerText = "Security Check"; pinDesc.innerText = "Enter PIN"; pinDesc.style.color = "var(--text-muted)";
                currentInput = ""; renderPinDots();
            } else { pinTimer.innerText = `Try again in ${left}s`; }
        }, 1000);
    };
    ...
    const handlePinSubmit = () => {
        ...
        if(pinMode === 'login' || pinMode === 'verify-action' || pinMode === 'remove') {
            if(currentInput === stored) {
                failedAttempts = 0; localStorage.setItem('pin-multiplier', '1');
                if(pinMode === 'login') {
                    pinScreen.style.display = 'none';
                    // CHECK FACE ID SETTING
                    if(localStorage.getItem('face-enabled') === 'true') {
                         setTimeout(() => startFaceID('verify'), 500); 
                    } else {
                         document.getElementById('main-app').style.display = 'block';
                         showToast("Welcome Back"); checkNotifications();
                    }
                } else { pinScreen.style.display = 'none'; if(pinCallback) pinCallback(); }
            } else {
                failedAttempts++; const maxAttempts = 3; const left = maxAttempts - failedAttempts;
                currentInput = ""; renderPinDots();
                if(left > 0) { pinDesc.innerText = `Incorrect Password. ${left} attempts left`; pinDesc.style.color = "var(--danger)"; showToast(`Incorrect PIN`); } 
                else { let mult = parseInt(localStorage.getItem('pin-multiplier') || '1'); let duration = 15 * mult; const lockUntil = new Date().getTime() + (duration * 1000); localStorage.setItem('pin-lock-until', lockUntil); localStorage.setItem('pin-multiplier', (mult * 2).toString()); startLockdownTimer(); }
            }
        } else if (pinMode === 'set') { tempPin = currentInput; openPin('confirm-set', pinCallback, "Confirm PIN"); } 
        else if (pinMode === 'confirm-set') {
            if(currentInput === tempPin) { localStorage.setItem('user-pin', currentInput); showToast("PIN Set Successfully"); pinScreen.style.display = 'none'; renderSettings(); if(pinCallback) pinCallback(); } 
            else { showToast("PIN Mismatch"); openPin('set', pinCallback); }
        }
    };

    // --- FACE ID & ENROLLMENT LOGIC ---
    const checkLighting = (vid) => {
        ...
        let colorSum = 0; for(let i=0; i<data.length; i+=4) { colorSum += (data[i] + data[i+1] + data[i+2]) / 3; }
        return Math.floor(colorSum / (data.length / 4));
    };

    const startFaceID = async (mode = 'verify') => {
        faceModal.style.display = 'flex'; faceGrid.style.opacity = '0';
        const title = document.getElementById('face-title');
        title.innerText = mode === 'enroll' ? "Face Enrollment" : "Biometric Scan";
        faceMsg.innerText = "Initializing Camera..."; faceMsg.style.color = "var(--primary)";
        ...
        try {
            ...
            video.onloadedmetadata = () => {
                video.play();
                faceMsg.innerText = "Checking Environment...";
                setTimeout(() => {
                    const brightness = checkLighting(video);
                    if(brightness < 40) {
                        faceMsg.innerText = "Access Denied: Too Dark"; faceMsg.style.color = "var(--danger)";
                        setTimeout(() => { stream.getTracks().forEach(t => t.stop()); faceModal.style.display = 'none'; if(mode==='verify') openPin('login', null, "Lighting Check Failed"); }, 2000);
                        return;
                    }
                    faceMsg.innerText = mode === 'enroll' ? "Scanning Features..." : "AI Analyzing: Identifying User...";
                    faceMsg.style.color = "#00e5ff"; faceGrid.style.opacity = '1';
                    
                    setTimeout(() => {
                        faceMsg.innerText = mode === 'enroll' ? "Enrollment Complete ✓" : "User Verified Successfully ✓";
                        faceMsg.style.color = "var(--success)";
                        
                        // IF ENROLLING, ENABLE SETTING NOW
                        if(mode === 'enroll') {
                            localStorage.setItem('face-enabled', 'true');
                            renderSettings(); // Update toggle UI
                        }
            ...
            console.error(err); faceMsg.innerText = "Camera Error. Manual Bypass."; faceMsg.style.color = "var(--danger)";
            setTimeout(() => { faceModal.style.display = 'none'; if(mode==='verify') document.getElementById('main-app').style.display = 'block'; }, 2000);
        }
    };

    document.addEventListener('app-ready-check-pin', () => {
        if(isLocked()) { startLockdownTimer(); } 
        else if(localStorage.getItem('user-pin')) { openPin('login', null, "Unlock App"); } 
        else { document.getElementById('main-app').style.display = 'block'; }
    });

    // --- PROFILE WITH INDEXEDDB ---
    ...
    const renderProfile = async () => {
        ...
        // Load Pic from IndexedDB
        const picId = localStorage.getItem('profile-pic-id');
        let picSrc = "https://user-gen-media-assets.s3.amazonaws.com/gpt4o_images/405ba96c-ee6d-452d-a99a-38ae2fd6b74b.png";
        if(picId) {
            const data = await dbHelper.get(picId);
            if(data) picSrc = data;
        }
        document.getElementById('dash-pic').src = picSrc; document.getElementById('profile-pic-detail').src = picSrc; 
        document.getElementById('dash-pic').onclick = () => viewImg(picSrc);
    };
    ...
    document.getElementById('btn-choice-rem').onclick = () => { localStorage.removeItem('profile-pic-id'); renderProfile(); document.getElementById('photo-choice-modal').style.display='none'; };
    ...
    const handleProfilePic = (e) => { 
        ...
        if(file) { 
            ...
            reader.onload = async (evt) => { 
                ...
                // Save to IndexedDB
                const id = await dbHelper.save(resized);
                localStorage.setItem('profile-pic-id', id);
    ...
    const setupCustomSelect = (wrapId, trigId, optId, txtId, optionsList, onSelect) => {
        const wrap = document.getElementById(wrapId); const options = document.getElementById(optId); const text = document.getElementById(txtId); options.innerHTML = '';
        optionsList.forEach(t => {
            const d = document.createElement('div'); d.className = 'custom-option'; d.innerText = t;
            d.onclick = () => { text.innerText = t; wrap.classList.remove('open'); onSelect(t); }; options.appendChild(d);
        });
        if(wrapId === 'doc-type-wrap') {
             ...
             c.onclick = () => { const val = prompt("Enter Category Name:"); if(val) { text.innerText = val; wrap.classList.remove('open'); onSelect(val); }}; options.appendChild(c);
        }
        document.getElementById(trigId).onclick = () => { document.querySelectorAll('.custom-select-wrap').forEach(el => { if(el.id !== wrapId) el.classList.remove('open'); }); wrap.classList.toggle('open'); };
    };
    ...
    const addTagRow = (k='', v='') => {
        ...
        document.getElementById('tags-container').appendChild(div); div.querySelector('.remove-tag').onclick = () => div.remove();
    }
    ...
    document.getElementById('save-doc-btn').onclick = async () => {
        ...
        let doc = { type: currentDocType, date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) };
        // Save Image to DB if exists
        if(tempDocImage) {
            const imgId = await dbHelper.save(tempDocImage);
            doc.imgId = imgId;
        }
        ...
        if(editingDocIndex >= 0) { 
            // Keep old ID if new one isn't provided
            if(!doc.imgId) doc.imgId = list[editingDocIndex].imgId; 
            doc.date = list[editingDocIndex].date; doc.time = list[editingDocIndex].time || doc.time; 
            list[editingDocIndex] = doc; showToast("Vault Updated"); 
    ...
    const openDocDetail = async (index) => {
        ...
        document.getElementById('btn-detail-del').onclick = () => {
            ...
            if(hasPin) { openPin('verify-action', () => { showConfirm("Permanent Delete", "Are you sure you want to delete this?", () => performDelete(index)); }, "Confirm PIN to Delete"); } 
            else { showConfirm("Permanent Delete", "Are you sure you want to delete this?", () => performDelete(index)); }
        };
        modal.style.display = 'flex';
    };
    document.getElementById('close-detail').onclick = () => document.getElementById('vault-detail-modal').style.display = 'none';

    const openEditMode = async (index, data) => {
        ...
        if(data.imgId) {
            tempDocImage = await dbHelper.get(data.imgId);
        } else {
            tempDocImage = data.image; // fallback
        }
        if(tempDocImage) document.getElementById('file-label').innerHTML = "<span style='color:var(--success)'>Image Kept (Tap to change)</span>";
    ...
    const performDelete = async (index) => {
        ...
        if(item.imgId) {
            await dbHelper.delete(item.imgId);
        }
        list.splice(index, 1);
        ...
        renderDocs(); showToast("Item Deleted Permanently");
    };

    let confirmCallback = null; const confirmModal = document.getElementById('confirm-modal');
    const showConfirm = (title, msg, cb) => { document.getElementById('confirm-head').innerText = title; document.getElementById('confirm-msg').innerText = msg; confirmCallback = cb; confirmModal.style.display = 'flex'; };
    document.getElementById('confirm-no').onclick = () => confirmModal.style.display = 'none'; document.getElementById('confirm-yes').onclick = () => { if(confirmCallback) confirmCallback(); };
    ...
    const renderExpenditure = () => {
        const list = JSON.parse(localStorage.getItem('user-expenses') || '[]'); const listContainer = document.getElementById('exp-history-list'); listContainer.innerHTML = ''; let totalBal = 0; let filteredUsed = 0; const now = new Date(); now.setHours(0,0,0,0);
        ...
        if(currentExpFilter === 'month') { const btn = document.createElement('button'); btn.className = 'action-btn'; btn.style.background = 'var(--primary)'; btn.style.color = 'black'; btn.style.marginTop = '15px'; btn.innerText = "View Full Monthly Report"; btn.onclick = () => renderMonthlyReport(filteredList); if(!document.getElementById('view-rpt-btn')) { btn.id = 'view-rpt-btn'; document.getElementById('exp-history-list').parentNode.appendChild(btn); } } else { const b = document.getElementById('view-rpt-btn'); if(b) b.remove(); }
    };
    ...
    const expModal = document.getElementById('expense-modal'); document.getElementById('btn-add-income').onclick = () => { currentExpType = 'credit'; document.getElementById('exp-modal-title').innerText = "Add Income"; document.getElementById('save-exp-btn').style.background = "var(--success)"; expModal.style.display = 'flex'; }; document.getElementById('btn-add-expense').onclick = () => { currentExpType = 'debit'; document.getElementById('exp-modal-title').innerText = "Add Expense"; document.getElementById('save-exp-btn').style.background = "var(--danger)"; expModal.style.display = 'flex'; }; document.getElementById('close-exp-btn').onclick = () => expModal.style.display = 'none'; document.getElementById('save-exp-btn').onclick = () => { const amt = document.getElementById('exp-amount-input').value; const desc = document.getElementById('exp-desc-input').value; if(!amt || amt <= 0) return showToast("Enter valid amount"); const list = JSON.parse(localStorage.getItem('user-expenses') || '[]'); list.push({ type: currentExpType, amount: amt, desc: desc, date: new Date().toLocaleDateString(), isoDate: new Date().toISOString() }); localStorage.setItem('user-expenses', JSON.stringify(list)); expModal.style.display = 'none'; renderExpenditure(); showToast("Saved"); document.getElementById('exp-amount-input').value = ''; };
    ...
    const renderSettings = () => { 
        ...
        const faceEnabled = localStorage.getItem('face-enabled') === 'true'; document.getElementById('toggle-face').checked = faceEnabled;
    };
    document.getElementById('toggle-pin').onclick = (e) => { e.preventDefault(); const hasPin = localStorage.getItem('user-pin'); if(hasPin) openPin('remove', () => { localStorage.removeItem('user-pin'); localStorage.removeItem('face-enabled'); renderSettings(); showToast("PIN & Face ID Removed"); }, "Enter PIN to Disable"); else openPin('set', null); };
    document.getElementById('btn-change-pin').onclick = () => { openPin('verify-action', () => { openPin('set', null); }, "Enter Old PIN"); };
    
    // Toggle Face ID
    document.getElementById('toggle-face').onclick = (e) => {
        e.preventDefault();
        const currentlyEnabled = localStorage.getItem('face-enabled') === 'true';
        if(currentlyEnabled) {
            localStorage.setItem('face-enabled', 'false');
            renderSettings(); showToast("Face ID Disabled");
        } else {
            // Must Enroll First
            startFaceID('enroll');
        }
